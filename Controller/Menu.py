...
class DataCreation:
    ...
    def dataframe_sorting_menu(self, dataframe_selected):
        ...
        while input_number != 7:
            input_number = int(input())
            if input_number == 1:
                self.panda_drop_na_menu(dataframe_selected)
            elif input_number == 2:
                ...
                self.sort_transposing_menu(dataframe_selected)
        self.dataframe_menu(dataframe_selected)

    def panda_drop_na_menu(self, dataframe_selected):
    ...
    def column_selection_menu(self, dataframe_selected):
        column_list = tuple(dataframe_selected.columns)
        count = 0
        column_index = {}
